...
export function positionStyle(position: { x: number, y: number } | undefined, options: { pieceSize: number }): JSX.CSSProperties {
    const corner: Corner = ((position?.y ?? 0) % 2 === 0)
        ? 'top-right'
        : 'top-left';
    const gridLocation = {
        row: position?.y ?? 0,
...
export const iconForPiece = (type: string | undefined): Component<JSX.SvgSVGAttributes<SVGSVGElement>> | undefined => {
    switch (type) {
        case 'ship':
            return ShipIcon;
        case 'plane':
...
export const colorForPiece = (owner: string | undefined): string | undefined => {
    switch (owner) {
        case 'red':
            return '#FF0000'; // Red
        case 'blue':
