...
export function positionStyle(
  ...
  const corner: Corner = position.y % 2 === 0 ? "top-right" : "top-left";
  const gridLocation = {
    row: position.y,
    col: position.x,
    corner,
  ...
  const left = intersectionX - options.size / 2 + boardPadding + 1.5;
  const top = intersectionY - options.size / 2 + boardPadding + 1.5;
  return {
    position: "absolute",
    left: `${left}px`,
    top: `${top}px`,
